...
from lottery.services import enter_raffle
...
class RaffleEnterAPIView(APIView):
    """
    POST /api/lottery/raffles/<id>/enter/
    Enter a raffle by spending tokens.
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, id):
        ...
        try:
            result = enter_raffle(request.user, raffle)
            
            return Response({
                "detail": "Successfully entered the raffle",
...
class RaffleTransparencyAPIView(APIView):
    ...
    def get(self, request, id):
        ...
        # Get all entry IDs in sorted order (as used in selection)
        ...
        transparency_data = {
            ...
            "verification_instructions": {
                "step1": "Combine seed and entry IDs: seed + ','.join(sorted_entry_ids)",
                "step2": "Generate SHA256 hash of the combined string",
                "step3": "Convert hash to integer: int(hash, 16) % entry_count",
                "step4": "Select entry at that index from sorted entries",
                "step5": "Verify the selected entry matches the winner",
            },
        }
